...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    // Used to transfer OculusControllerState from the websocket to stdout.
    let (tx, mut rx): (
        Sender<OculusControllerState>,
        Receiver<OculusControllerState>,
    ) = channel(1);
    ...
    let receiver = tokio::spawn(async move {
        while let Some(state) = rx.recv().await {
            ...
            let servo = (PULSE_MIN_US as f32)
                + ((stick + 1f32) * 50f32 * ((PULSE_MAX_US - PULSE_MIN_US) as f32) / 100f32);
    ...
    let routes = warp::path("ws")
        ...
        .and(warp::ws())
        .and(add_tx)
        .map(|ws: warp::ws::Ws, tx: Sender<OculusControllerState>| {
